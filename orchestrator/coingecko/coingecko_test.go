...
func TestFeeThreshodTwoDecimals(t *testing.T) {
	...
	currentTokenPrice := QueryUSDPrice(injTokenContract) // "usd":9.35
	...
	isFeeLimitExceeded := CheckFeeThreshod(injTokenContract, totalFeeInINJ, minFeeInUSD)
...
func TestFeeThreshodNineDecimals(t *testing.T) {
	...
	currentTokenPrice := QueryUSDPrice(shibTokenContract) // "usd":0.000008853
	...
	isFeeLimitExceeded := CheckFeeThreshod(shibTokenContract, totalFeeInSHIB, minFeeInUSD)
	assert.True(t, isFeeLimitExceeded, "FeeAccumulated is less than ExpectedFee")
