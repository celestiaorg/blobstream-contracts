...
contract DAVerifierTest is DSTest {
    ...
    function testVerifySharesToDataRootTupleRoot() public {
        ...
        NamespaceMerkleMultiproof[]
            memory _shareProofs = new NamespaceMerkleMultiproof[](1);
        ...
        (bool valid, DAVerifier.ErrorCodes errorCode) = DAVerifier
            .verifySharesToDataRootTupleRoot(bridge, sharesProof);
        assertTrue(valid);
    ...
    function testComputeSquareSizeFromRowProof() public {
        (
            ...
            BinaryMerkleTree.ErrorCodes error
        ) = BinaryMerkleTree.verify(
                fixture.dataRoot(),
                fixture.getRowRootToDataRootProof(),
                fixture.firstRowRoot()
            );
        ...
        (uint256 actualSquareSize, DAVerifier.ErrorCodes errorCode) = DAVerifier
    ...
    function testComputeSquareSizeFromShareProof() public {
        ...
        // check that the computed square size is correct
        uint256 expectedSquareSize = 1;
        uint256 actualSquareSize = DAVerifier.computeSquareSizeFromShareProof(
            fixture.getShareToRowRootProof()
        );
        assertEq(actualSquareSize, expectedSquareSize);
    }
    ...
    function testValidSlice() public {
        ...
        (bytes[] memory result, DAVerifier.ErrorCodes error) = DAVerifier.slice(
            data,
            1,
            3
        );
    ...
    function testOutOfBoundsSlice() public {
        ...
        assertEq(
            uint256(error),
            uint256(DAVerifier.ErrorCodes.OutOfBoundsRange)
        );
