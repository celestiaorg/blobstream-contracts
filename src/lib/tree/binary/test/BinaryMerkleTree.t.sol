...
contract BinaryMerkleProofTest is Test {
    ...
    function testMultiproof() public {
        ...
        // The proof range has to cover exactly the leaves that were passed in. Without that
        // check an extra leaf is silently ignored and still verifies.
        bytes[] memory extraLeaves = new bytes[](leaves.length + 1);
        for (uint256 i = 0; i < leaves.length; i++) {
            extraLeaves[i] = leaves[i];
        }
        extraLeaves[leaves.length] = "leaf that is not in the tree";
        assertTrue(!BinaryMerkleTree.verifyMulti(dataroot, multiproof, extraLeaves));
    }

    /// An empty proof range consumes no leaf, so the root would be rebuilt from the side nodes
    /// alone and any data would verify against it.
    function testMultiproofRejectsEmptyRange() public {
        ...
        data[0] = "data that is not in the tree";
        data[1] = "neither is this";

        BinaryMerkleMultiproof memory emptyRange = BinaryMerkleMultiproof(sideNodes, 4, 4);
        assertTrue(!BinaryMerkleTree.verifyMulti(root, emptyRange, data));

        BinaryMerkleMultiproof memory invertedRange = BinaryMerkleMultiproof(sideNodes, 5, 4);
        assertTrue(!BinaryMerkleTree.verifyMulti(root, invertedRange, data));
    }
}
