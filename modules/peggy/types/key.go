...
func GetFeeSecondIndexKey(fee sdk.Coin, injTokenContract string) []byte {
	var contractAddr string
	if fee.Denom == "inj" {
		contractAddr = injTokenContract
	} else {
		er, _ := ERC20FromPeggyCoin(fee)
		contractAddr = er.Contract
	}

	r := make([]byte, 1+ETHContractAddressLen+8)
	copy(r[0:], SecondIndexOutgoingTXFeeKey)
	copy(r[len(SecondIndexOutgoingTXFeeKey):], []byte(contractAddr))
	copy(r[len(SecondIndexOutgoingTXFeeKey)+len(contractAddr):], sdk.Uint64ToBigEndian(fee.Amount.Uint64()))
	return r
}
